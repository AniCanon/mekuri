...
public struct MekuriFoldGeometry: Equatable, Sendable {
    public let pageWidth: CGFloat
    public let configuration: MekuriConfiguration

    public init(pageWidth: CGFloat, configuration: MekuriConfiguration) {
        self.pageWidth = pageWidth
        self.configuration = configuration
    }

    /// Horizontal position of the fold axis. Out-of-range progress clamps to 0...1.
    public func foldAxisOffset(progress: CGFloat) -> CGFloat {
        self.pageWidth * (1 - min(max(progress, 0), 1))
    }

    public func isFlat(progress: CGFloat) -> Bool {
        progress <= 0
    }

    /// Full-height band centred on the fold axis.
    public func creaseShadowRect(progress: CGFloat, pageSize: CGSize) -> CGRect {
